...
import CrearGrupoApp from '../CreateGroup';
...
const Profile = () => {
  ...
  return (
    <div className="wrapper" >
      <div className="one" >
        <ProfileHeader name={"user2.username"} email={"user2.email"} photo={""} secondary_photo={""}/>
      </div>
        {vista === '1' && adminGroups.map(e => <Profilegroups  name={"hola mundo"} logo={""} top={520} left={"52rem"}/>)}
        {vista === '2' && memberGroups.map(e => <Profilegroups  name={e.name} logo={""} top={280} left={"52rem"}/>)}
      <div className="two">  
      <Radio.Group className="selector-groups" onChange={handleChange} value={vista}>
          <Radio.Button className="groups-boton-1" value="1">Grupos que administro</Radio.Button>
          <Radio.Button className="groups-boton-2" value="2">Grupos a los que pertenezco</Radio.Button>
